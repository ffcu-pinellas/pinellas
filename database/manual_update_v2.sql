...
-- Add 'pin' column to user_cards if not exists
-- Only run this if the column does not exist. MySQL does not support IF NOT EXISTS for ADD COLUMN directly in standard syntax without a procedure, 
-- but you can run this safely if 'pin' doesn't exist. If it does, it will error harmlessly in most manual imports or you can ignore.
-- Alternatively, use this safer block if your MySQL version supports it (8.0+):
-- ALERT: If you are using an older MySQL, just run: 
-- ALTER TABLE `user_cards` ADD COLUMN `pin` varchar(255) COLLATE utf8mb4_unicode_ci NULL AFTER `status`;

SET @dbname = DATABASE();
SET @tablename = "user_cards";
SET @columnname = "pin";
