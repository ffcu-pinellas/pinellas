...
class PageController extends Controller
{
    ...
    {
        $languages = Language::where('status', true)->get();
        $landingContent = LandingContent::where('id', $id)->orWhere('locale_id', $id)->get();
        ...
        $groupData = $groupData->map(function ($items) {
            return $items->first()->only(['id', 'icon', 'title', 'description', 'type', 'photo', 'locale_id']);
        })?->toArray();
    ...
    public function contentUpdate(Request $request)
    {
        ...
        $locale = $input['locale'];
        $landingContent = LandingContent::where('id', $input['id'])->where('locale', $locale)->first();
        $engLandingContent = LandingContent::where('id', $input['locale_id'])->where('locale', '=', 'en')->first();
        ...
        $data = [
            ...
            'title' => $input['title'],
            'locale_id' => $engLandingContent->id,
            'description' => $request->get('description'),
        ];
