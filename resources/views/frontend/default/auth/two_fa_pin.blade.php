...
@section('content')
    <h3>{{ __('Multi-Factor Authentication') }}</h3>
    
    <div style="text-align: center; color: var(--body-text-secondary-color); font-size: 15px; margin-bottom: 25px; line-height: 1.5;">
        {{ __('Please enter the 6-digit security code generated by your authenticator app.') }}<br>
    ...
    <form method="POST" action="{{ route('user.setting.2fa.verify') }}">
        ...
        <div class="input-group">
            <label for="one_time_password">{{ __('Security Code') }}</label>
